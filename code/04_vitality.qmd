...
```{r}
province_counts <- polentometro_en_include |> 
  count(Region, Province) |> 
  arrange(Region, Province, n)

write_csv(province_counts, "data/derived/province-counts.csv")
```
...
materials |>
  mutate(materials = str_remove(materials, "materials_")) |> 
  ggplot(aes(generation, fill = value)) +
  geom_bar(position = "fill") +
