...
#| label: languages
polentometro_page15 |> 
  pivot_longer(c(languages_parents, first_langacq, spoke_to_children, would_teach_children, children_speak)) |> 
  drop_na(value) |> 
  filter(
    !(value %in% c("No_minors", "No_adult_children", "Neither")),
  ) |> 
  mutate(
    name = factor(name, levels = c("languages_parents", "first_langacq", "spoke_to_children", "children_speak", "would_teach_children")),
...
## Factor 3: Proportion of speakers

```{r}
#| label: prop-speak-plot
polentometro_page15 |> 
