...
speak_counts_region <- polentometro_en_include |>
...
responses_comuni <- left_join(adm3, geo_counts_comune, by = c("NAME_2" = "Province", "NAME_3" = "Comune"))

speakers_regions <- left_join(adm1, speak_counts_region, by = c("NAME_1" = "Region"))
...
ggplot() +
  geom_sf(data = adm0) +
  geom_sf(data = responses_comuni, aes(fill = n)) +
  geom_sf(data = adm1, fill = NA, colour = "red") +
...
ggsave("img/comune-counts.png", width = 7, height = 5)
```
...
```{r}
#| label: speaker-regions

ggplot() +
  geom_sf(data = adm0) +
  geom_sf(data = speakers_regions, aes(fill = prop)) +
  geom_sf(data = adm1, fill = NA, colour = "red") +
...
ggsave("img/speaker-region.png", width = 7, height = 5)
```
...
```{r}
#| label: speaker-comuni-2

ggplot() +
  geom_sf(data = adm0) +
  geom_sf(data = speakers_comuni |> filter(NAME_3 %in% c("Torino", "Milano", "Verona", "Genova", "Bologna")), aes(fill = prop)) +
  geom_sf(data = adm1, fill = NA, colour = "red") +
  scale_fill_distiller(palette = "YlGnBu", direction = 1, limits = c(0, 1)) +
  coord_sf(c(6.5, 14), c(43.5, 47)) +
  labs(x = element_blank(), y = element_blank()) +
